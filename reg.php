...
if ($_POST)
{
	...
	$ExistUser = doquery("SELECT `username` FROM {{table}} WHERE `username` = '" . ($_POST['character']) . "' LIMIT 1;", 'users', TRUE);
	if ($ExistUser)
	{
	...
	$ExistMail = doquery("SELECT `email` FROM {{table}} WHERE `email` = '" . ($_POST['email']) . "' LIMIT 1;", 'users', TRUE);
	if ($ExistMail)
	{
	...
	else
	{
		...
		$QryInsertUser .= "`username` = '" . strip_tags($UserName) . "', ";
		...
		$NewUser = doquery("SELECT `id` FROM {{table}} WHERE `username` = '" . $_POST['character'] . "' LIMIT 1;", 'users', TRUE);
