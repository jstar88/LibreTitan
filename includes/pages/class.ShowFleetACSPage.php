...
class ShowFleetACSPage
{
	function __construct ( $CurrentUser , $CurrentPlanet )
	{
		...
		{
			$added_user_id 	= 0;
			$member_qry 		= doquery("SELECT `id` FROM {{table}} WHERE `username` ='".($_POST['addtogroup'])."' ;",'users');
			...
			if ( $added_user_id > 0 )
			{
				$new_eingeladen_mr = ($_POST['acs_invited']).','.$added_user_id;
