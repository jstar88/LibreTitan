        ...
        if ($CurrentUser['ally_id'] == 0)
        {
            ...
            if ($mode == 'make' && $CurrentUser['ally_request'] == 0)
            {
                if ($yes == 1 && $_POST)
                {
                    ...
                    doquery("UPDATE {{table}} SET
								...
								WHERE `id`='" . intval($CurrentUser['id']) . "'", "users");

                    $page = $this->message_box(str_replace('%s', $_POST['atag'], $lang['al_created']), str_replace('%s', $_POST['atag'], $lang['al_created']) . "<br/><br/>", "", $lang['al_continue']);
                } else
                {
            ...
            if ($mode == 'apply' && $CurrentUser['ally_request'] == 0)
            {
                ...
                } else
                {
                    ...
                    {
                        doquery("UPDATE {{table}} SET `ally_request`='" . intval($allyid) . "' , ally_request_text='" . (strip_tags($_POST['text'])) . "' , ally_register_time='" . time() . "' WHERE `id`='" . $CurrentUser['id'] . "'", "users");
        ...
        if ($CurrentUser['ally_id'] != 0 && $CurrentUser['ally_request'] == 0)
        {
            ...
            if ($mode == 'admin' && $edit == 'ally')
            {
                ...
                if ($_POST)
                {
                    ...
                    {
                        $_POST['owner_range'] = ($_POST['owner_range']);
                        $_POST['web'] = ($_POST['web']);
                        $_POST['image'] = ($_POST['image']);
                        //		$_POST['text'] 				=  ( $_POST['text'] );
                        $_POST['text'] = str_replace("&lt;br /&gt;", "", (strip_tags(($_POST["text"]))));
                    }
                }

                if ($_POST['options'])
                {
                    $ally['ally_owner_range'] = ((strip_tags($_POST['owner_range'])));
                    $ally['ally_web'] = ((strip_tags($_POST['web'])));
                    $ally['ally_image'] = ((strip_tags($_POST['image'])));
            ...
            if ($mode == 'admin' && $edit == 'name')
            {
                ...
                {
                    $ally['ally_name'] = (strip_tags($_POST['nombre']));
            ...
            if ($mode == 'admin' && $edit == 'tag')
            {
                ...
                {
                    doquery("UPDATE {{table}} SET `ally_tag` = '" . (strip_tags($_POST['etiqueta'])) . "' WHERE `id` = '" . $CurrentUser['ally_id'] . "';", 'alliance');
                }
            ...
            if ($mode == 'admin' && $edit == 'transfer')
            {
                if (isset($_POST['newleader']))
                {
                    ...
                    doquery("UPDATE {{table}} SET `ally_owner`='" . (strip_tags($_POST['newleader'])) . "' WHERE `id`=" . intval($CurrentUser['ally_id']) . "", 'alliance');
                    doquery("UPDATE {{table}} SET `ally_rank_id`='0' WHERE `id`='" . (strip_tags($_POST['newleader'])) . "' ", 'users');
                    exit(header("location:game.php?page=alliance", 2));
                }
                if ($ally['ally_owner'] != $CurrentUser['id'])
                {
