...
function ResetUniverse ( $CurrentUser )
{
	global $phpEx;
		...
		$AllUsers  = doquery ("SELECT `username`,`password`,`email`, `email_2`,`authlevel`,`universe`,`galaxy`,`system`,`planet`, `dpath`, `onlinetime`, `register_time`, `id_planet` FROM {{table}} WHERE 1;", 'users_s');
		...
		while ( $TheUser = mysql_fetch_assoc($AllUsers) )
		{
			if ( $TheUser['onlinetime'] > $LimitTime )
			{
				...
				if ($UserPlanet['name'] != "")
				{
					...
					$QryInsertUser .= "`universe` = '".        $TheUser['universe']        ."', ";
					...
					CreateOnePlanetRecord ($TheUser['universe'], $TheUser['galaxy'], $TheUser['system'], $TheUser['planet'], $NewUser['id'], $UserPlanet['name'], true);
