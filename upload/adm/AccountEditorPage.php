		...
		if ($_POST)
		{
			...
			if ($id != NULL)
			{
				...
				if ($delete != 'on')
				{
					...
					if ($change_pos == 'on')
					{
						if (is_numeric($universe) &&is_numeric($galaxy) && is_numeric($system) && is_numeric($planet) && $galaxy > 0 && $system > 0 && $planet > 0 && $universe <= MAX_UNIVERSE_IN_WORLD &&
							$galaxy <= MAX_GALAXY_IN_WORLD && $system <= MAX_SYSTEM_IN_GALAXY && $planet <= MAX_PLANET_IN_SYSTEM)
						{
							$Queryyy	=	doquery("SELECT * FROM {{table}} WHERE `universe` = '".$universe."' AND `galaxy` = '".$galaxy."' AND `system` = '".$system."' AND
							...
							if ($P['planet_type'] == '1')
							{
								if (!$Queryyy)
								{
									...
									{
										doquery ("UPDATE {{table}} SET `universe` = '".$universe."', `galaxy` = '".$galaxy."', `system` = '".$system."', `planet` = '".$planet."' WHERE
										`universe` = '".$P['universe']."' AND `galaxy` = '".$P['galaxy']."' AND `system` = '".$P['system']."' AND `planet` = '".$P['planet']."' AND `planet_type` = '3'", "planets");
									}
							...
							elseif ($P['planet_type'] == '3')
							{
								if ($Queryyy)
								{
									if ($Queryyy['id_luna'] == '0')
									{
										doquery ("UPDATE {{table}} SET `id_luna` = '0' WHERE `universe` = '".$P['universe']."' AND `galaxy` = '".$P['galaxy']."' AND `system` = '".$P['system']."' AND
											`planet` = '".$P['planet']."'", "galaxy");

										doquery ("UPDATE {{table}} SET `universe` = '".$universe."', `galaxy` = '".$galaxy."', `system` = '".$system."', `planet` = '".$planet."',
										`id_luna` = '".$id."' WHERE `id_planet` = '".$Queryyy['id_planet']."'", "galaxy");

										$QMOON2	=	doquery("SELECT * FROM {{table}} WHERE `universe` = '".$universe."' AND `galaxy` = '".$galaxy."' AND `system` = '".$system."' AND
							...
							$Log	.=	$lang['log_change_pla_pos'].$Name.": [".$universe.":".$galaxy.":".$system.":".$planet."]\n";
						}
						else
						{
