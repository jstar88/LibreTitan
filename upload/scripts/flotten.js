...
function target() {
  ...
  universe = document.getElementsByName("universe")[0].value;
	...
	return("["+universe+":"+galaxy+":"+system+":"+planet+"]");
}
...
function setTarget(universe, galaxy, solarsystem, planet, planettype) {
	document.getElementsByName('universe')[0].value = universe;
...
function setTargetLong(universe, galaxy, solarsystem, planet, planettype, mission, cnt) {
	setTarget(universe, galaxy, solarsystem, planet, planettype);
	setMission(mission);
	setUnions(cnt);
}
...
function distance() {
  ...
  var targetGalaxy;
	...
	var ff=document.getElementsByName("thisuniverse")[0];
	var tt=document.getElementsByName("universe")[0];
  if(ff == null || tt == null){
    thisUniverse = 0; 
    targetUniverse = 0;
  }
  else{
    thisUniverse = ff.value;
    targetUniverse = tt.value;
    }
  ...
  targetGalaxy = document.getElementsByName("galaxy")[0].value;
	...
	dist = 0;
	if ((targetUniverse - thisUniverse) != 0) {
		dist += Math.abs(targetUniverse - thisUniverse) * 10000;
	} 
...
function consumption() {
	...
	for (i = 200; i < 220; i++) {
		if (document.getElementsByName("ship" + i)[0]) {
			spd = 35000 / (dur * speedfactor - 10) * Math.sqrt(dist * 10 / msp);
...
function setPlanet(string) {
	...
	document.getElementsByName('universe')[0].value = splitstring[0];
	document.getElementsByName('galaxy')[0].value = splitstring[1];
	document.getElementsByName('system')[0].value = splitstring[2];
	document.getElementsByName('planet')[0].value = splitstring[3];
	document.getElementsByName('planettype')[0].value = splitstring[4];
...
//	thisuniverse = document.getElementsByName("thisuniverse")[0].value;
  thisgalaxy = document.getElementsByName("thisgalaxy")[0].value;
	thissystem = document.getElementsByName("thissystem")[0].value;
	thisplanet = document.getElementsByName("thisplanet")[0].value;
	...
	for (i = 0; i < cnt; i++) {
		...
		targetuniverse = document.getElementsByName('union'+i+'universe')[0] ;
    if(targetuniverse== null)
      targetuniverse=0;
    else
      targetuniverse=targetuniverse.value;
		...
		if (targetuniverse == universe && targetgalaxy == galaxy && targetsystem == system && targetplanet == planet && targetplanettype == planettype){


			inSpeedLimit = isInSpeedLimit(flightTime(thisuniverse, thisgalaxy, thissystem, thisplanet,
			targetuniverse,targetgalaxy, targetsystem, targetplanet,
...
function flightTime(universe, galaxy, system, planet,
targetuniverse,targetgalaxy, targetsystem, targetplanet,
spd, maxspeed, speedfactor) {
	//    alert ("flighttime called 1"+galaxy+" "+system+" "+planet+" "+targetgalaxy+" "+targetsystem+" "+targetplanet);

	if ((universe - targetuniverse) != 0) {
		dist = Math.abs(universe - targetuniverse) * 10000;
	}
...
function showCoords() {
	...
	document.getElementsByName('universe')[0].disabled = false;
...
function hideCoords() {
	...
	document.getElementsByName('universe')[0].disabled = true;
