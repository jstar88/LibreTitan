...
function display2($groupName,$groupMainTemplate,$topnav = true, $metatags = '', $menu = true)
{
    global $user, $planetrow, $xgp_root,$game_config,$dpath;
    ...
    Xtreme::assignToGroup('main_page','header',"html/$folder/header");
    Xtreme::assignToGroup('main_page','footer',"html/$folder/footer");
    if(!empty($groupName))
        Xtreme::assignGroupToGroup($groupName,$groupMainTemplate,'main_page','body');
    else
        Xtreme::assignToGroup('main_page','body',$groupMainTemplate);
    Xtreme::assign('title',$game_config['game_name']);
    Xtreme::assign('meta',$metatags);
    Xtreme::assign('dpath',$dpath);
        if(true){
            if ($topnav)
        {
            $page=new ShowTopNavigationBar($user, $planetrow);
            $page->show();
        }
        if (true)
        {
            $page=new ShowLeftMenu($user);
            $page->show();
           
        }
        if ($user['urlaubs_modus'] == 0)
            PlanetResourceUpdate($user, $planetrow, CURRENT_TIME);
        else
    ...
    $DisplayPage = Xtreme::outputGroup('main_page', "html/$folder/page");
    if ($user['authlevel'] == 3 && $game_config['debug'] == 1 && !defined('ADMIN'))
    {
...
function isUniverseInWar($universe1, $universe2)
{
    ...
    else
    {
        ...
        {
            if (($war['id_universe_in_war1'] == $universe1 && $war['id_universe_in_war2'] == $universe2 && $war['on_time_to_war'] == 1) || ($war['id_universe_in_war1'] == $universe2 && $war['id_universe_in_war2'] == $universe1 && $war['on_time_to_war'] == 1))
                return 1;
        }
        return 0;
    }
...
function universesInWarWith($universe)
{
    ...
    foreach ($war_config as $war)
    {
        ...
        if ($war['id_universe_in_war1'] == $universe && $war['on_time_to_war'] == 1)
        {
            ...
            if ($war['id_universe_in_war2'] == $universe && $war['on_time_to_war'] == 1)
            {
...
function updateWars()
{
    ...
    foreach ($war_config as & $war)
    {
        if (time() >= ($war['war_start']) && time() <= ($war['war_end']))
        {
...
function warWithThis($universe)
{
    ...
    foreach ($war_config as $war)
    {
        if ($war['id_universe_in_war1'] == $universe || $war['id_universe_in_war2'] == $universe)
        {
