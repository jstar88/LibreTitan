...
function ShowFleetShortcuts($CurrentUser)
{
	...
	if (isset($_GET['mode']))
	{
		if ($_POST)
		{
			...
			$r = strip_tags(mysql_real_escape_string($_POST['n'])) . "," .intval($_POST['u']).",". intval($_POST['g']) . "," . intval($_POST['s']) . "," . intval($_POST['p']) . "," . intval($_POST['t']) . "\r\n";
			$CurrentUser['fleet_shortcut'] .= $r;
			doquery("UPDATE {{table}} SET fleet_shortcut='".mysql_real_escape_string($CurrentUser['fleet_shortcut'])."' WHERE id=".intval($CurrentUser['id'])."", "users");
		...
		$page = "<div id=\"content\"><form method=POST><table border=0 cellpadding=0 cellspacing=1 width=519>
				...
				<input type=text name=u value=\"$u\" size=3 maxlength=1 title=\"".$lang['fl_shortcut_universe']."\">
	...
	elseif (isset($_GET['a']))
	{
		...
		if ($CurrentUser['fleet_shortcut'])
		{
			...
			$page = "<div id=\"content\"><form method=POST><table border=0 cellpadding=0 cellspacing=1 width=519>
					<tr height=20>
					<td colspan=2 class=c>". $lang['fl_shortcut_edition'] ."{$c[0]} [{$c[5]}:{$c[1]}:{$c[2]}:{$c[3]}]</td>
					</tr>";
			$page .= "<tr height=\"20\"><th>
					<input type=hidden name=a value=$a>
					<input type=text name=n value=\"{$c[0]}\" size=32 maxlength=32>
					<input type=text name=u value=\"{$c[5]}\" size=3 maxlength=3>
