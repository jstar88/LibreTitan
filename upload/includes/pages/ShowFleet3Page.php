...
class ShowFleet3Page {
	...
	public function show() {
		...
		if ($_POST['mission'] != 15) {
			...
			elseif ($fleetmission == 9 && mysql_num_rows($select) < 1) exit (header("location:game." . $phpEx . "?page=fleet"));
		}
		...
		if ($fleetmission == 9) {
			...
			if ($TargetPlanet['id_owner'] == '')
				$HeDBRec = $MyDBRec;
			elseif ($TargetPlanet['id_owner'] != '') $HeDBRec = doquery("SELECT `id`,`onlinetime`,`ally_id`,`urlaubs_modus` FROM {{table}} WHERE `id` = '" . intval($TargetPlanet['id_owner']) . "';", 'users', true);
	...
	private function generalControll(&$info) {
		global $resource,$xgp_root, $game_config ;
	...
	private function expeditionControll(& $info) {
		...
		elseif ($ExpeditionEnCours >= $EnvoiMaxExpedition) message("<font color=\"red\"><b>" . $lang['fl_expedition_fleets_limit'] . "</b></font>", "game." . $phpEx . "?page=fleet", 2);
	...
	private function sanitizecoordinates() {
		$args = array (
			'universe' => array (
				$targetUniverse => FILTER_CALLBACK,
				array (
					'options' => array (
						$this,
						'validateUniverse'
			...
			'system' => array (
				...
				'options' => array (
					'min_range' => 1,
					'max_range' => MAX_SYSTEM_IN_GALAXY
				)
			),
			'planet' => array (
				'filter' => FILTER_VALIDATE_INT,
				'options' => array (
					'min_range' => 1,
					'max_range' => MAX_PLANET_IN_SYSTEM
				)
			),
			'mission' => array (
				$mission => FILTER_CALLBACK,
				array (
					'options' => array (
						$this,
						'validateMission'
