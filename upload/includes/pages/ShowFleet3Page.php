...
class ShowFleet3Page
{
    ...
    private function generalControll($info)
    {
        global $resource;
        //the target planet can't be the start planet
        if ($this->CurrentPlanet['universe'] == $info['universe'] && $this->CurrentPlanet['galaxy'] == $info['galaxy'] && $this->CurrentPlanet['system'] == $info['system'] && $this->CurrentPlanet['planet'] == $info['planet'] && $this->CurrentPlanet['planet_type'] == $info['planettype'])
        ...
        if ($info['target_planet']['destruyed'] != 0)
    ...
    private function attackControll(&$info)
    {
    ...
    private function stayAllyControll(&$info)
    {
        //check if the target planet exist
        if (empty($info['target_planet']))
            exit(header("location:game." . $phpEx . "?page=fleet"));
    ...
    private function missionSender($idMission)
    {
        global $missions;
        $funcName = $mission[$idMission] . 'Sender';
        $this->$funcName($in_war);
    }
    private function attackSender($in_war)
    {

    }
    private function acsSender($in_war)
    {
        $fleet_group_mr = 0;

    }
    private function transportSender($in_war)
    {

    }
    private function staySender($in_war)
    {

    }
    private function stayAllySender($in_war)
    {
        $StayDuration = $_POST['holdingtime'] * 3600;
        $StayTime = $fleet['start_time'] + $_POST['holdingtime'] * 3600;
    }
    private function spySender($in_war)
    {

    }
    private function colonisationSender($in_war)
    {

    }
    private function recyclingSender($in_war)
    {
        $YourPlanet = false;
        $UsedPlanet = false;
        //start mod
    ...
    private function destructionSender($in_war)
    {

    }
    private function mipSender($in_war)
    {

    }
    private function expeditionSender($in_war)
    {
    ...
    public function show()
    {
        ...
        $info = array();
        $info['in_war'] = $in_war;
        $info['target_planet'] = doquery("SELECT * FROM {{table}} WHERE `universe` = '$universe' AND`galaxy` = '$galaxy' AND `system` = '$system' AND planet` = '$planet' AND `planet_type` = '$planettype';", 'planets', true);

        $this->generalControll($info);
        $this->missionControll($_POST['mission'], $info);
        $this->missionSender($_POST['mission'], $info);
        ...
        if ($fleetmission == 9)
        {
            ...
            {
                if ($MyGameLevel > ($HeGameLevel * $protectionmulti) && $TargetPlanet['id_owner'] != '' && ($_POST['mission'] == 1 or $_POST['mission'] == 6 or $_POST['mission'] == 9) && $protection == 1 && $HeGameLevel < $protectiontime)
                    message("<font color=\"lime\"><b>" . $lang['fl_week_player'] . "</b></font>", "game." . $phpEx . "?page=fleet", 2);

                if (($MyGameLevel * $protectionmulti) < $HeGameLevel && $TargetPlanet['id_owner'] != '' && ($_POST['mission'] == 1 or $_POST['mission'] == 5 or $_POST['mission'] == 6 or $_POST['mission'] == 9) && $protection == 1 && $MyGameLevel < $protectiontime)
            ...
            $AllFleetSpeed = GetFleetMaxSpeed($fleetarray, 0, $this->CurrentUser);
            ...
            $QryInsertFleet .= "`fleet_owner` = '" . intval($this->CurrentUser['id']) . "', ";
            ...
            $this->CurrentPlanet['metal'] -= $TransMetal;
            $this->CurrentPlanet['crystal'] -= $TransCrystal;
            $this->CurrentPlanet['deuterium'] -= $TransDeuterium + $consumption;
            $this->CurrentPlanet['population'] -= $TransPopulation;
            $this->CurrentPlanet['population_free'] -= $TransPopulation;
    ...
    private function sanitizecoordinates()
    {
        ...
        'mission'           => array($mission => FILTER_CALLBACK, array('options' => array($this,'validateMission'))),
        'fleet_group'       => array('filter' => FILTER_VALIDATE_INT)
        );      
