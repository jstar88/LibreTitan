<?php
//finish
class MissionCaseStayAlly extends FlyingFleetHandler
{
   public function MissionCaseStayAlly($FleetRow)
	{
		...
		if (!$this->isMissionEnded($FleetRow))
		{
            ...
            $Message = sprintf($real_language_starter['sys_tran_mess_owner'], $TargetName, GetTargetAdressLink($FleetRow, ''),
				...
				$Message = sprintf( $real_language_target['sys_tran_mess_user'], $StartName, GetStartAdressLink($FleetRow, ''),
			...
			elseif($this->isStayEnded($FleetRow))
			{
				$QryUpdateFleet  = "UPDATE {{table}} SET ";
