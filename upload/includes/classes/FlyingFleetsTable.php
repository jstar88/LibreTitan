...
class FlyingFleetsTable
{
	...
	public function BuildFleetEventTable($FleetRow, $Status, $Owner, $Label, $Record)
	{
		...
		//start mod
    $StartPlanet    = doquery("SELECT `name` FROM {{table}} WHERE 
		`universe` = '".intval($FleetRow['fleet_start_universe'])."' AND 
    `galaxy` = '".intval($FleetRow['fleet_start_galaxy'])."' AND 
    `system` = '".intval($FleetRow['fleet_start_system'])."' AND 
    `planet` = '".intval($FleetRow['fleet_start_planet'])."' AND 
    `planet_type` = '".intval($FleetRow['fleet_start_type'])."';", 'planets', true);
		$StartType      = $FleetRow['fleet_start_type'];
		$TargetPlanet   = doquery("SELECT `name` FROM {{table}} WHERE
    `universe` = '".intval($FleetRow['fleet_end_universe'])."' AND  
    `galaxy` = '".intval($FleetRow['fleet_end_galaxy'])."' AND 
    `system` = '".intval($FleetRow['fleet_end_system'])."' AND 
    `planet` = '".intval($FleetRow['fleet_end_planet'])."' AND 
    `planet_type` = '".intval($FleetRow['fleet_end_type'])."';", 'planets', true);
		//end mod
